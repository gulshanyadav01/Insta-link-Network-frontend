...
const ProfileItem = ({profile:{ user:{_id, name, avatar}, status, company}}) => {
    return (
        <div className ="w-50 ml-12 m-4 h-52 border-t-2 border-gulshan-dascar bg-gulshan-dascar  shadow rounded hover:border-2 hover:border-limegreen-600">
                <div className = "w-48 rounded-t h-8  bg-gradient-to-r from-gulshan-1000 via-pink-500 to-gulshan-1100 flex-column">
                    {/* <h1 className = "w-20 h-20 border-white border-4 rounded-full bg-cerise-500 hover:bg-cerise-600  m-auto"> </h1> */}
                    <Link to = {`/profile/${_id}`}> <img src = {Logo} alt = "images"  className = "w-20 h-20 border-limegreen-400 border-4 rounded-full m-auto"/></Link>
                    {/* <h1 className = "ml-5 mr-5 text-royalblue-400 capitalize font-bold hover:underline  ">gulshan yadav</h1>*/}
                    <h1 className = "ml-12  mt-4 text-royalblue-400 capitalize font-bold hover:underline  ">{name}</h1>
                    <h1 className = "ml-12 mt-2 text-royalblue-400 capitalize  hover:underline  ">{status}</h1>
